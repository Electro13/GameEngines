...
Ray CollisionDetection::ScreenPosToWorldRay(glm::vec2 mouseCoords_, glm::vec2 screenSize_,
	Camera* camera_) {
...
bool CollisionDetection::RayObbIntersection(Ray* ray_, BoundingBox* box_) {
	...
	if (fabs(h) > 0.001f) {
		float t1 = (g + box_->minVert.y) / h;
		float t2 = (h + box_->maxVert.y) / h;
	...
	if (fabs(j) > 0.001f) {
		float t1 = (i + box_->minVert.z) / j;
		float t2 = (j + box_->maxVert.z) / j;
